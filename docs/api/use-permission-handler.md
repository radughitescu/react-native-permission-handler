...
| `requestFullAccess()` | `() => Promise<PermissionStatus>` | Upgrade from `limited` to `granted` via `engine.requestFullAccess()`, then re-checks and updates the hook state. Supported on the RNP engine for photos (`react-native-permissions` >= 5.5.1) and contacts (>= 5.6.0), on the Expo engine when `expo-media-library` or `expo-contacts` is installed, and on the testing engine. Throws if the resolved engine does not implement `requestFullAccess`. See [limited-photo recipe](../recipes/limited-photo-upgrade.md) and [limited-contacts recipe](../recipes/limited-contacts.md). |
